...
def occupancy_trend():
    ...
    try:
        ...
        # Query ParkingUsageLog table for each day (all parking sessions)
        from .models import ParkingUsageLog
        result = {d: 0 for d in days_str}
        
        logger.info(f"Querying parking usage logs for dates: {days_str}")
        
        # Get actual parking session counts from database
        # Count sessions that started on each day (entry_time)
        usage_logs = db.session.query(
            func.date(ParkingUsageLog.entry_time).label('date'),
            func.count(ParkingUsageLog.id).label('count')
        ).filter(
            ParkingUsageLog.entry_time >= days[0],
            ParkingUsageLog.entry_time < today + timedelta(days=1)
        ).group_by(func.date(ParkingUsageLog.entry_time)).all()
        
        logger.info(f"Found parking sessions: {usage_logs}")
        ...
        # Also count current reservations (active bookings) for today
        from .models import Reservation
        today_reservations = db.session.query(
            func.date(Reservation.parking_timestamp).label('date'),
            func.count(Reservation.id).label('count')
        ).filter(
            Reservation.parking_timestamp >= today,
            Reservation.parking_timestamp < today + timedelta(days=1),
            Reservation.leaving_timestamp.is_(None)  # Only active reservations
        ).group_by(func.date(Reservation.parking_timestamp)).all()
        
        logger.info(f"Found active reservations for today: {today_reservations}")
        
        # Add active reservations to today's count
        for row in today_reservations:
            date_str = str(row.date)
            if date_str in result:
                result[date_str] += row.count
        ...
        response_data = {
            ...
            'total_sessions': sum(result.values()),
